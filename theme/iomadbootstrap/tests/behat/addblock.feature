...
Feature: Add a block using iomadbootstrap theme [iomadbootstrap]
  ...
  Scenario: All the expected blocks are displayed in the Add a block list for iomadbootstrap
  ...
  Scenario: Admins can change unaddable blocks using the unaddableblocks setting for iomadbootstrap
    Given the following config values are set as admin:
      | unaddableblocks | online_users,private_files,settings | theme_iomadbootstrap|
    And I am on "Course 1" course homepage with editing mode on
    When I click on "Add a block" "select"
    Then I should not see "Online users"
    And I should not see "Private files"
    # The settings block is defined as required block for iomadbootstrap, so it will be displayed always.
