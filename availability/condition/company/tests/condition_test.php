...
class condition_test extends \advanced_testcase {
    ...
    public function test_usage() {
        ...
        // Add user to companys and refresh cache.
        companys_add_member($company1, $user);
        companys_add_member($company2, $user);
        ...
        // Admin user doesn't belong to a company, but they can access it
        ...
        $information = $cond->get_description(false, false, $info);
        $information = \core_availability\info::format_info($information, $course);
        $this->assertMatchesRegularExpression('~You belong to.*\(Missing company\)~', $information);
    }
    ...
    public function test_constructor() {
        ...
        $structure->id = 123;
        $cond = new condition($structure);
        $this->assertEquals('{company:#123}', (string)$cond);

        // Valid (no id).
        unset($structure->id);
        $cond = new condition($structure);
        $this->assertEquals('{company:any}', (string)$cond);
    }
    ...
    public function test_filter_users() {
        ...
        // 1: in company 1.
        // 2: in company 2.
        companys_add_member($company1, $students[1]);
        companys_add_member($company2, $students[2]);
